    ...
    def run(self, state: TradingState):
        ...
        for product, order_depth in state.order_depths.items():
            ...
            if len(trader_data[product]["expected price"]) == 0:
                ...
                d = (trader_data[product]["mid price"][-1] - trader_data[product]["expected price"][-1])*(-0.2988)
            ...
            if pos < position_limit:
                orders.append(Order(product, min(best_bid+1, int(np.floor(acceptable_price))), position_limit-pos))
            ...
            if pos > -position_limit:
                orders.append(Order(product, max(best_ask-1, int(np.ceil(acceptable_price))), -pos-position_limit))

            # if cur_pos > 0:
            #     orders.append(Order(product, int(np.ceil(acceptable_price+1.5)), -cur_pos-(position_limit-position_margin)))
            # elif cur_pos < 0:
            #     orders.append(Order(product, int(np.floor(acceptable_price)-1.5), -cur_pos+(position_limit-position_margin)))
