...
PARAMS = {
    ...
    'GIFT_BASKET':{        
        ...
        # 'take_position_stage': [0, 60],
        # 'take_price_spread': [(3, 3), (3, 1)],
        'make_position_stage': [0, 10],
        'make_price_spread': [(1, 1), (1, 2)],
        'make_price_offset': [3, 3],
    },
    ...
    #     'price_method': 'average',
    #     'price_data_size': 1,
    #     'strategy': ['market_take'],
    #     'take_position_stage': [100],
    #     'take_price_spread': [(-100, -100)],
...
class Trader:
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]: 
        ...
        # Modify the state to include only selected products
        state.listings = {k: v for k, v in state.listings.items() if k in PRODUCTS}
        state.order_depths = {k: v for k, v in state.order_depths.items() if k in PRODUCTS}
        state.own_trades = {k: v for k, v in state.own_trades.items() if k in PRODUCTS}
        state.market_trades = {k: v for k, v in state.market_trades.items() if k in PRODUCTS}
        state.position = {k: v for k, v in state.position.items() if k in PRODUCTS}
        state.observations.conversionObservations = {k: v for k, v in state.observations.conversionObservations.items() if k in PRODUCTS}
        ...
        for product in PRODUCTS:
            if (product not in state.listings) or (product not in trader_data): continue
