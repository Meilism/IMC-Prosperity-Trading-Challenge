...
class Trader:
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:        
        ...
        buy_power = position_limit - position # Maximum amount of AMETHYSTS to buy from current position
        sell_power = position_limit + position # Maximum amount of AMETHYSTS to sell from current position
        orders = []
            ...
            if len(order_depth.buy_orders) > 0:
                for bid, bid_amount in order_depth.buy_orders.items():
                    if bid > trader_data_prev["AMETHYSTS"]["acceptable_price"]:
                        bid_amount = min(bid_amount, sell_power)
                        orders.append(Order("AMETHYSTS", bid, -bid_amount))
                        sell_power -= bid_amount
                    elif bid == trader_data_prev["AMETHYSTS"]["acceptable_price"] and buy_power == 0:
                        bid_amount = min(bid_amount, sell_power)
                        orders.append(Order("AMETHYSTS", bid, -bid_amount))
                        sell_power -= bid_amount
                    else:
            ...
            if len(order_depth.sell_orders) > 0:
                for ask, ask_amount in order_depth.sell_orders.items():
                    if ask < trader_data_prev["AMETHYSTS"]["acceptable_price"]:
                        ask_amount = min(-ask_amount, buy_power)
                        orders.append(Order("AMETHYSTS", ask, ask_amount))
                        buy_power -= ask_amount
                    elif ask == trader_data_prev["AMETHYSTS"]["acceptable_price"] and sell_power == 0:
                        ask_amount = min(-ask_amount, buy_power)
                        orders.append(Order("AMETHYSTS", ask, ask_amount))
                        buy_power -= ask_amount
                    else:
        ...
        for buy_price_diff, buy_fraction in trader_data_prev["AMETHYSTS"]["buy_fraction"]:
            buy_quantity = int(buy_fraction * buy_power)
            if buy_quantity > 0:
        ...
        for sell_price_diff, sell_fraction in trader_data_prev["AMETHYSTS"]["sell_fraction"]:
            sell_quantity = int(sell_fraction * sell_power)
            if sell_quantity > 0:
        ...
        # Compute mid price and update acceptable price
