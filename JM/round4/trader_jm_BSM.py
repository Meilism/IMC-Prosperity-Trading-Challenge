...
class Trader:
    ...
    def updateData(self, product: Symbol, state: TradingState, data: Dict[str, Any]) -> None:
        ...
        elif PARAMS[product]['price_method'] == 'basket':
            price = PARAMS[product]['basket_offset']
            for p in PARAMS[product]['PRODUCT']:
                _, _, p_mid_price = self.getBestBidAsk(p, state)
                if p_mid_price is None: return
                price += PARAMS[product]['PRODUCT'][p] * p_mid_price
            data['expected_mid_price'] = price

        
        # Use Black Scholes model to calculate mid_price and hedge factor
        elif PARAMS[product]['price_method'] == 'Black_Scholes':
            base_product = PARAMS[product]['base_product']
            base_price = self.getBestBidAsk(PARAMS[product]['base_product'], state)[2]
            data['expected_mid_price'], PARAMS[product]['PRODUCT'][base_product], _ = self.getBlackScholes(base_price)
    ...
    def computePairTrading(self, product: Symbol, state: TradingState) -> Dict[Symbol, List[Order]]:
        ...
        for p in PARAMS[product]['PRODUCT']:
            ...
            pos_diff  = round(prod_position + position * PARAMS[product]['PRODUCT'][p])
    ...
    def computeThresholdOrders(self, product: Symbol, state: TradingState, data: Dict[str, Any]) -> List[Order]:
        ...
        best_bid, best_ask, mid_price = self.getBestBidAsk(product, state)
        basket_price = data['expected_mid_price']

        if mid_price is None: return orders
        
        if (mid_price - basket_price > PARAMS[product]['trade_offset']):
        ...
        elif (mid_price - basket_price < -PARAMS[product]['trade_offset']):
